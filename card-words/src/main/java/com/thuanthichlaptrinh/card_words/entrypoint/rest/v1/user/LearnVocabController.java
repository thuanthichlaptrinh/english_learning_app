...
public class LearnVocabController {
    ...
    @GetMapping("/vocabs/topic-id/{topicId}")
    @Operation(summary = "Lấy từ vựng để học theo Topic ID (có phân trang)", description = """
                    Lấy danh sách từ vựng để HỌC theo Topic ID cụ thể (tương tự như endpoint theo topic name nhưng sử dụng Long ID).
                    Bao gồm từ chưa học và từ có status NEW hoặc UNKNOWN.

                    **Thuật toán:**
                    1. Lấy từ có status = NEW hoặc UNKNOWN trong topic (ưu tiên UNKNOWN)
                    2. Nếu chưa đủ, lấy thêm từ chưa học trong topic
                    ...
                    **Khi nào dùng:**
                    - Học từ vựng theo chủ đề cụ thể với Long ID của topic
                    - Tránh vấn đề case-sensitive với tên topic
                    - Type-safe hơn khi frontend sử dụng ID
            """, security = @SecurityRequirement(name = "Bearer Authentication"))
    public ResponseEntity<ApiResponse2<List<ReviewVocabResponse>>> getLearnVocabsByTopicId(
            @AuthenticationPrincipal User user,
            @Parameter(description = "ID của topic", example = "1") @PathVariable Long topicId,
            @Parameter(description = "Số trang (bắt đầu từ 1)", example = "1") @RequestParam(defaultValue = "1") Integer page,
            @Parameter(description = "Số lượng từ trên mỗi trang", example = "20") @RequestParam(defaultValue = "20") Integer size) {

        PagedReviewVocabResponse response = learnVocabService.getVocabsForLearningByTopicId(user, topicId,
                page, size);
        ...
        return ResponseEntity.ok(ApiResponse2.success(
                "Lấy danh sách từ vựng để học theo topic ID thành công",
                response.getVocabs(),
                meta));
    }
